...
<!-- SYSTEME DE VERIFICATION DU STATUT VENDEUR -->
<script>
let accountDisabledShown = false;
let sellerStatusTimer = null;
...
async function checkSellerStatus() {
  try {
    const response = await fetch(`api_check_seller_status.php?_t=${Date.now()}`, {
      cache: 'no-store',
      headers: {
    ...
    if (data && data.is_active === false) {
      handleSellerDisabled(data);
    }
  } catch (err) {
    console.error('Erreur verification statut vendeur:', err);
  }
}

function handleSellerDisabled(data) {
  if (accountDisabledShown) return;
  accountDisabledShown = true;

  if (sellerStatusTimer) {
    clearInterval(sellerStatusTimer);
    sellerStatusTimer = null;
  }

  const redirectTo = data && data.redirect_to ? data.redirect_to : 'account_disabled.php';
  const message = data && data.message
    ? data.message
    : "Votre compte vendeur est desactive par l'administrateur.";
  const agentName = data && data.name ? data.name : 'Vendeur';

  showAccountDisabledModal(message, agentName, redirectTo);
}

function showAccountDisabledModal(message, agentName, redirectTo) {
  if (!document.getElementById('accountDisabledModal')) {
    const modalHTML = `
          ...
          <div class="modal-content" style="border: none; border-radius: 20px; box-shadow: 0 20px 60px rgba(0, 48, 135, 0.15);">
            <div class="modal-header" style="background: linear-gradient(135deg, #dc2626, #b91c1c); border: none; border-radius: 20px 20px 0 0; padding: 24px;">
              <div style="text-align: center; width: 100%; color: #fff;">
                ...
                <h5 class="modal-title" style="color: #fff; font-weight: 700; font-size: 20px;">Compte desactive</h5>
              </div>
            </div>
            <div class="modal-body" style="padding: 32px; text-align: center;">
              <p style="font-size: 16px; color: #0b1f3a; margin-bottom: 16px; font-weight: 600;">
                ${safeHtml(message)}
              </p>
              <p style="font-size: 14px; color: #6b7280; margin-bottom: 8px;">
                L'acces a cet espace sera ferme automatiquement.
              </p>
              <p style="font-size: 13px; color: #9ca3af; margin: 0;">
                <i class="fa-solid fa-user-circle"></i> ${safeHtml(agentName)}
              </p>
            </div>
            <div class="modal-footer" style="border-top: 1px solid #e5e7eb; padding: 16px 24px;">
              <button type="button" class="btn" style="background:#0070e0;color:#fff;border:none;border-radius:24px;padding:10px 24px;font-weight:600;" onclick="window.location.href='${safeHtml(redirectTo)}'">
  ...
  const modalEl = document.getElementById('accountDisabledModal');
...
sellerStatusTimer = setInterval(checkSellerStatus, 4000);
checkSellerStatus();
</script>
</body>
</html>
